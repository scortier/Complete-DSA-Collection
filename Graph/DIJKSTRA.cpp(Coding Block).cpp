...
public:
    ...
    void printAdj() {
        ...
        // j.first = key and j.second = list
        for (auto j : m) {

            cout << j.first << "->";

            //Iterater over the list of cities
            //l.first = name of cities; l.second = weight
            for (auto l : j.second) {
    ...
    void dijsktraSSSP(T src) {
        //for evey node mainatain a dist array.dist<city,int>
        ...
        while (!s.empty()) {
            ...
            auto p =   *(s.begin()); //s.begin() is a ptr and ptr of ptr will give pair.
            T node = p.second;
            ...
            for (auto childPair : m[node]) {
                ...
                if (nodeDist + childPair.second < dist[childPair.first]) {
                    ...
                    // to get the shortest dist to reach node from root node
